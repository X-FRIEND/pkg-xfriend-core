...
mocha.describe('Base Function tests', function () {
  ...
  mocha.it('Test database connect with error', (done) => {
    ...
    //STUB de banco de dados com sucesso
    sinon.stub(mongoose, 'connect').withArgs(process.env.MONGO_URI).returns(new Promise(function (resolve, reject) {
  ...
  mocha.it('Test if isFtp and subProcessId is repassed to the logger', () => {
    lib = new BlankFunction({ contextProperties: { "isFtp": true, "subProcessId": 123 } });
  ...
  mocha.it('Test if recall step function is repassed to the logger', () => {
    lib = new BlankFunction({ contextProperties: { "recallStepFunction": true } });
  ...
  mocha.it('Test callback error', () => {
    ...
    const logSpy = sinon.spy(BlankFunction.prototype, "finishFunction");
    lib = new BlankFunction({ contextProperties: { processId: 0, recallStepFunction: true } });
    ...
    expect(logSpy.args[0][1].contextProperties.recallStepFunction).to.be.equal(true);
  ...
  mocha.it('Test if the "Session" variable are going passed foward', (done) => {
    ...
    lib = new BlankFunction({ contextProperties: { "processOwner": 1, "partnerQueue": "QUEUENAME", "subProcessId": 1 } });
    lib.finish(null, {});

    expect(logSpy.args[0][1].contextProperties.processOwner).to.be.equal(1);
    expect(logSpy.args[0][1].contextProperties.partnerQueue).to.be.equal("QUEUENAME");
