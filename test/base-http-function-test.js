...
const tokenLogin = require("../lib/services/tokenLogin");

mocha.describe.skip("Base Http Function tests", function () {
  var sinon;
  let lib;
  ...
  mocha.it("Test if context.succeed is called", () => {
    ...
    expect(logSpy.args[0][0]).to.be.deep.equal(
      lib.getApiGatewayResponse(200, lib.processId, data)
    );
  });

  mocha.it("Test callback error", () => {
    const logSpy = sinon.spy(
      BlankHttpFunction.prototype,
      "finishFunctionWithError"
    );
  ...
  mocha.it("Test run with customer token", (done) => {
    ...
    sinon.stub(mongoose.connection, "close").returns(
      new Promise(function (resolve) {
        return resolve();
      })
    );

    sinon.stub(tokenLogin, "getToken").returns(
      new Promise(function (resolve) {
    ...
    lib.event = {
      queryStringParameters: {
        accessToken:
          "YOUj2xqwEwiAiPt0ehDXl8MziizB2wMlIyCGhItdqmjBlzuiFsAFZLSMaG6nHKVl",
        signature: 172193417,
      },
    };

    lib.context.succeed = function (result) {
      expect(logSpy.getCall(0).returnValue).to.be.equal(
        "YOUj2xqwEwiAiPt0ehDXl8MziizB2wMlIyCGhItdqmjBlzuiFsAFZLSMaG6nHKVl"
      );
