...
module.exports = class AmazonService {
  static sendQueue(value, arg_prefix) {
    ...
    if (!this.canCallAmazonFunction())
      return Promise.resolve("Local does not call Amazon");
  ...
  static sendQueueBatch(value, url) {
    ...
    var params = { Entries: value, QueueUrl: url };
    ...
    return new Promise(function (resolve, reject) {
      stepFuncion.startExecution(params, function (err, data) {
        if (err) {
          logger.error(
            util.format(
              "Calling stepFuncions.startExecution function %s error",
  ...
  static findOrCreateQueue(arg_prefix) {
    ...
    logger.log(
      util.format("Calling findOrCreateQueue: %s", arg_prefix),
      "debug"
    );

    var params = {
      QueueName: util.format(
        "%s_%s_%s",
        arg_prefix,
        process.env.SUFIXQUEUE,
        process.env.STAGE
      ),
      Attributes: {},
    };
    ...
    return new Promise(function (resolve, reject) {
      if (selfSqs == null && selfSqs.createQueue != null) {
        ...
        selfSqs.createQueue(params, function (err, data) {
          if (err) {
            logger.error(
              util.format("Calling createQueue function: %s", params),
              err
            );
    ...
    return new Promise(function (resolve, reject) {
      stepFuncion.stopExecution(params, function (err) {
        if (err) {
          logger.error(
            util.format(
              "Calling stepFuncions.stopExecution function %s error %s",
              params.executionArn,
              "error"
            ),
            err
        ...
        logger.log(
          util.format(
            "Calling stepFuncions.stopExecution success %s",
            params.executionArn
          ),
          "debug"
        );
  ...
  static sendMailv2(params) {
    const {
      destinationsEmail,
      replyToAddresses,
      sourceEmail = "noreplyapp@x-friend.com.br",
      subject,
      body,
      attachment,
      fileName,
    ...
    return new Promise(function (resolve, reject) {
      ses.sendEmail(emailParams, (err, data) => {
        ...
        logger.log("EMAIL: ", JSON.stringify(destinationsEmail), "debug");
        resolve(data);
      });
    });
  }
  ...
  static sendMail(emaillist, subject, body) {
    let eParams = {
      ...
      Source: "noreplyapp@x-friend.com.br",
    };
    ...
    return new Promise(function (resolve, reject) {
      ...
      ses.sendEmail(eParams, function (err, data) {
        if (err) {
          logger.error(
            util.format("Sending email  %s error %s", emaillist, "error"),
            err
          );
          return reject(err);
        } else {
    ...
    return new Promise(function (resolve, reject) {
      selfSqs.getQueueAttributes(params, function (err, data) {
        if (err) {
          logger.error(
            util.format(
              "Deatil Queue function %s error %s",
              params.QueueUrl,
              "error"
            ),
            err
        ...
        logger.log(
          util.format("Deatil Queue success %s", params.QueueUrl),
          "debug"
        );
        return resolve(data);
      });
...
module.exports.Rekognition = class AmazonService {
  async detectText(bucket, imageName) {
    ...
    try {
      ...
      response = detectText.TextDetections.map((t) => {
        return t.DetectedText;
      });
    } catch (err) {
      logger.error(
        util.format("Rekognition.detectText function %s error %s", err)
      );
      throw err;
    }
    return response;
