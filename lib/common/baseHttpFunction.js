...
class BaseHttpFunction extends BaseFunction {
  ...
  finishFunction(response, isAsync) {
    ...
    if (!isAsync) {
      self.context.succeed(
        self.getApiGatewayResponse(httpStatusCode.OK, this.processId, response)
      );
    } else {
      return self.getApiGatewayResponse(
        httpStatusCode.OK,
        this.processId,
        response
      );
  ...
  getAccessTokenValue() {
    if (self.event != null && self.event.headers != null) {
      if (self.event.headers["accessToken"] != null)
        return self.event.headers["accessToken"];
      if (self.event.headers["AccessToken"] != null)
        return self.event.headers["AccessToken"];
    }

    if (self.event != null && self.event.queryStringParameters != null) {
      if (self.event.queryStringParameters.accessToken != null)
        return self.event.queryStringParameters.accessToken;
      if (self.event.queryStringParameters.AccessToken != null)
        return self.event.queryStringParameters.AccessToken;
    }
  ...
  getSignatureValue() {
    if (self.event != null && self.event.queryStringParameters != null) {
      if (self.event.queryStringParameters.signature != null)
        return self.event.queryStringParameters.signature;
      if (self.event.queryStringParameters.Signature != null)
        return self.event.queryStringParameters.Signature;
    }
