...
const qaDataService = (() => {
  const instance = axios.create({
    baseURL:
      process.env.URL_QADATA || "http://nginx-x-frienddigital/api/bfbmsqadata",
  ...
  const getScenarioById = async ({ scenarioId }) => {
    const response = await instance.get(`/v1/scenarios/${scenarioId}`, {
      params: scenarioId,
    });
    return response.data;
  };
  ...
  const getScenarioExecution = async ({ scenarioId }) => {
    const response = await instance.get(
      `/v1/scenarios/${scenarioId}/execution?rows=1`,
      { params: scenarioId }
    );
  ...
  const getScenarioReservation = async ({ scenarioId }) => {
    const response = await instance.get(
      `/v1/scenarios/${scenarioId}/reservations`,
      { params: scenarioId }
    );
    return response.data;
  };
  ...
  const getReservationById = async ({ reservationId }) => {
    const response = await instance.get(`/v1/reservations/${reservationId}`, {
