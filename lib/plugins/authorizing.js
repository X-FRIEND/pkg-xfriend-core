...
const Authorizing = () => {
  ...
  const softAuthenticate = (request, nextAction) =>
    authenticate(request, nextAction, true);

  const hardAuthenticate = (request, nextAction) =>
    authenticate(request, nextAction, false);

  const xFriendSoftTokenScheme = () => ({
    credentials: null,
    isValid: false,
    authenticate: softAuthenticate,
  });

  const xFriendHardTokenScheme = () => ({
    credentials: null,
    isValid: false,
    authenticate: hardAuthenticate,
  });

  const isValidSignature = (signatureId, credentials) => {
    if (!signatureId) return true;

    const signature = credentials.signatureList.filter(
      (id) => id === signatureId
    );
  ...
  const logError = ({ request, error, response }) =>
  ...
  const authenticate = async (request, nextAction, continueRequest) => {
    ...
    try {
      const credentials = await authorizer.get({
        token: authorization,
        consumerSystem,
        userId,
      });
      ...
      return nextAction.response(badGateway).code(statusBadGateway).takeover();
    }
  };
  ...
  const setupPlugin = async (server) => {
    ...
    await server.auth.strategy("x-friend-soft-authorizer", softStrategyName, {
      xFriendSoftTokenScheme,
    });
    await server.auth.strategy("x-friend-hard-authorizer", hardStrategyName, {
      xFriendHardTokenScheme,
    });
  };
