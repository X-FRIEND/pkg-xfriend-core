...
const loggerHandler = ({
  ...
  try {
    ...
    const headersNormalized = utility.normalizeProperties(
      headers || get("headers", event)
    );
    ...
    const logObject = {
      ...
      queryStringParameters:
        queryStringParameters || get("queryStringParameters", event),
      userAgent: get("user-agent", headersNormalized),
      consumerKey: get("consumer-key", headersNormalized),
      headers: headersNormalized || JSON.stringify(headersNormalized),
